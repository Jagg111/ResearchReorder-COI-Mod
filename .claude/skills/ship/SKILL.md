...
3. Run `git log $prevTag..HEAD --pretty=format:%s` (or `git log HEAD --pretty=format:%s` if no tags)
4. Show the user the raw commit list
...
Using the commit list from Step 2, write player-friendly bullet points. Rules:
- Write for players, not developers. "Fixed: queue resets on save load" not "refactor queue persistence layer"
- Omit commits that have no player-visible effect (build changes, README edits, code cleanup, comment fixes)
- Merge closely related commits into one bullet if cleaner
- Each bullet leads with a past-tense verb or label: "Fixed:", "Added:", "Improved:" — or just a plain verb
- Convert any `#N` issue references to markdown links: `[#N](https://github.com/Jagg111/COI-ResearchQueue/issues/N)`
