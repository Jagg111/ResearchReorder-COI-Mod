...
8. **`OnOrderChanged` index coordinate space:** Both `oldIndex` and `newIndex` are final visual positions (0-based) in the n-item list — not insertion points. Max value is `n-1`. The event only fires when position actually changes. These values map directly to queue indices when the scroll container holds only queue rows.
9. **Reorderable DOM layout during drag:** Internally the manipulator reparents the dragged item into an absolute-positioned `dragTargetContainer` and inserts a `shadowSpace` placeholder. The container has `n+1` DOM children during drag (n-1 real items + shadow + absolute dragTargetContainer always last). Shadow can only land at indices 0..n-1.
10. **Drag constraint index math — post-pop coordinate space:** `OnOrderChanged(oldIndex, newIndex)` maps directly to `PopAt(oldIndex)` + `EnqueueAt(item, newIndex)`. After `PopAt`, the queue has `n-1` items; `EnqueueAt(item, n-1)` appends safely (documented in `Queueue`). When computing valid drag ranges against a post-pop tempQueue: inserting at tempQueue index `i` shifts the item currently at `i` to `i+1`, so a dependent at index `i` is still after the inserted item — the constraint is `latest = i`, NOT `i-1`. The append-to-end position is `tempQueue.Count` (not `Count-1`). Getting either off by one causes false clamping where the error sound fires despite no real dependency violation (fixed in issue #13).
